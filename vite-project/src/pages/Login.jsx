...
// Verificar si el usuario ya tiene un token válido al cargar el componente
    useEffect(() => {
 ...
 // Validación de campos
    ...
    try {
      ...
      storage.setItem('fakestore_token', token) // va guardar un valor token en el localStorage o sessionStorage dependiendo de rememberMe
      storage.setItem('fakestore_user', user.username)
      storage.setItem('fakestore_email', user.email)
 // Despues que se valida todo, se redirige a la página de inicio
      navigate('/home')
      // En cado de error, se muestra un mensaje al usuario
    } catch (error_) {
      setError(error_.message || 'Error al iniciar sesión. Intenta nuevamente.')
    } finally {
      setLoading(false)// Se desactiva el estado de carga después de intentar iniciar sesión
    }
}
